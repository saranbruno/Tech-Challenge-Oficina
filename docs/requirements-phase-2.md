...
| --- | --- | --- | --- | --- | --- | --- |
| F2-ARQ-01 | Aplicar Clean Code com nomes claros, responsabilidades coesas, contratos concretos e sem alterar comportamentos validos da Fase 1. | O Dia 3 separou as 22 operacoes CRUD em casos de uso, removeu o fluxo legado de abertura e tipou composicoes e metricas. Restam 12 assinaturas `mixed` de paginacao e mapeamento, alocadas integralmente no Dia 4. | `app/Domain/**`, `app/Application/**`, convencoes em `docs/architecture.md`. | Diff de refatoracao, regressao HTTP, suite completa e cobertura sem reducao. | 2, 3 e 4 | Parcial |
| F2-ARQ-02 | Aplicar Clean Architecture com dependencia `Interface HTTP -> Aplicacao -> Dominio`; Eloquent, Laravel, HTTP e fornecedores permanecem fora do Dominio. | O Dominio esta conforme; o Dia 3 eliminou as quatro chamadas a `config()` e o retorno de `App\Models\User` na Aplicacao. Paginadores Eloquent ainda atravessam contratos `mixed` e serao corrigidos e protegidos por testes no Dia 4. | Contratos internos, adapters de Infraestrutura e testes em `tests/Architecture/**`. | Mapa de dependencias, teste arquitetural e suite funcional aprovada. | 2, 3 e 4 | Parcial |
