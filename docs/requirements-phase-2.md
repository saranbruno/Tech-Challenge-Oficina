...
| --- | --- | --- | --- | --- | --- | --- |
| F2-ARQ-01 | Aplicar Clean Code com nomes claros, responsabilidades coesas, contratos concretos e sem alterar comportamentos validos da Fase 1. | Dominio e Aplicacao ja estao separados; auditoria formal ainda nao executada. | `app/Domain/**`, `app/Application/**`, convencoes em `docs/architecture.md`. | Diff de refatoracao, regressao HTTP, suite completa e cobertura sem reducao. | 2, 3 e 4 | Parcial |
| F2-ARQ-02 | Aplicar Clean Architecture com dependencia `Interface HTTP -> Aplicacao -> Dominio`; Eloquent, Laravel, HTTP e fornecedores permanecem fora do Dominio. | Quatro camadas existem e o Eloquent esta em `app/Infrastructure`; protecao automatizada ainda nao existe. | Contratos internos, adapters de Infraestrutura e testes em `tests/Architecture/**`. | Mapa de dependencias, teste arquitetural e suite funcional aprovada. | 2, 3 e 4 | Parcial |
