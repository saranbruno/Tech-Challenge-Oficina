...
| F2-NOT-03 | Usar Mailpit e SMS fake/log localmente e em CI; permitir SMTP e Twilio em producao somente por configuracao e Secrets. | Mailpit, adapter de e-mail, template e SMTP configuravel foram implementados; SMS local e Twilio configuravel foram implementados no Dia 8. | Integracao com transicoes no Dia 9 e Compose completo no Dia 17. | E-mail entregue no Mailpit por teste opt-in; SMS log/fake observavel, testes sem rede paga, configuracao Twilio exigida pelo adapter e secrets ausentes do Git. | 7, 8, 9 e 17 | Parcial |
...
| F2-K8S-02 | Executar PostgreSQL no Kubernetes com StatefulSet, Service interno, PVC, probes e credenciais externas ao codigo. | PostgreSQL existe somente no Compose. | Modulo Terraform de PostgreSQL e integracao com o cluster. | Banco Ready, API conectada pelo Service, PVC associado e dados preservados apos recriar o pod. | 21, 23, 26 e 27 | Parcial |
| F2-K8S-03 | Usar HPA `autoscaling/v2` com CPU e memoria simultaneamente, Metrics Server e requests/limits compativeis. | Metrics Server 0.9.0 integrado ao Terraform local/CI; HPA autoscaling/v2 configurado com CPU 70%, memoria 80% e 1 a 4 replicas. | Targets reais validados no Dia 22 (CPU 6%/70%, memoria 50%/80%, uma replica); demonstrar escalabilidade sob carga no Dia 27. | `kubectl top`, targets conhecidos de CPU/memoria, scale-up e retorno ao minimo sob carga; evidencias no Dia 22 de `docs/project-progress.md`. | 22 e 27 | Parcial |
...
| F2-IMG-01 | Publicar imagem publica no GHCR com tag imutavel por SHA, tag auxiliar `fase-2`, labels OCI e pull anonimo comprovado. | O workflow reutilizavel de publicacao, os labels OCI e a substituicao controlada de imagem nos overlays foram preparados no Dia 24. O pacote ainda nao foi publicado nem validado anonimamente. | Executar o workflow apos versionar as alteracoes, configurar a visibilidade publica do pacote e usar a tag SHA no deploy. | Publicacao, pull anonimo, healthcheck e identificacao do SHA implantado. | 24 e 26 | Parcial |
| F2-CI-01 | Em PR e push, executar dependencias, build, PostgreSQL, migrations, Pint, testes, cobertura, OpenAPI, audit, Terraform, Kubernetes e build Docker sem publicar em PR. | Workflow de CI preparado com PostgreSQL efemero, cobertura PCOV, validacoes e build sem push; ainda nao executado no GitHub Actions. | `.github/workflows/ci.yml` e protecao de branch exigindo o job de validacao. | Job verde e falha controlada bloqueando o pipeline, sem secrets em logs. | 25 | Parcial |
| F2-CD-01 | No runner hospedado, publicar imagem, criar Kind temporario, provisionar PostgreSQL, aplicar configuracao e manifests, migrar, aguardar rollouts, fazer smoke, coletar evidencias e destruir sempre. | Workflow de CD preparado com CI reutilizavel, publicacao GHCR, Kind temporario, PostgreSQL Terraform, Secrets efemeros, migrations, smoke, artefatos e destruicao no mesmo job; ainda nao executado no GitHub Actions. | `.github/workflows/cd-kind.yml`, Terraform e overlay `ci`. | Logs e artefatos de cada etapa; `terraform destroy` em condicao `always`. | 26 | Parcial |
| F2-DOC-01 | Manter OpenAPI/Swagger, README, arquitetura, DDD, notificacoes, infraestrutura e CI/CD sincronizados apenas com o implementado. | OpenAPI 3.1 valida e documenta 37 operacoes da Fase 1; documentos arquiteturais existem. | Documentos incrementais e revisao completa. | Lint OpenAPI, Swagger executavel, diagramas renderizados, links e comandos testados. | 1, 15 e 28 a 30 | Parcial |
