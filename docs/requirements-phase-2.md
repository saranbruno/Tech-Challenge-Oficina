...
| --- | --- | --- | --- | --- | --- | --- |
| F2-ARQ-01 | Aplicar Clean Code com nomes claros, responsabilidades coesas, contratos concretos e sem alterar comportamentos validos da Fase 1. | Auditoria do Dia 2 encontrou quatro servicos CRUD com 22 operacoes independentes, um fluxo legado duplicado de abertura, arrays de contrato implicito e 18 assinaturas `mixed`; cada ponto possui item fechado nos Dias 3 ou 4. | `app/Domain/**`, `app/Application/**`, convencoes em `docs/architecture.md`. | Diff de refatoracao, regressao HTTP, suite completa e cobertura sem reducao. | 2, 3 e 4 | Parcial |
| F2-ARQ-02 | Aplicar Clean Architecture com dependencia `Interface HTTP -> Aplicacao -> Dominio`; Eloquent, Laravel, HTTP e fornecedores permanecem fora do Dominio. | O Dominio esta conforme; a Aplicacao possui quatro chamadas a `config()` e um contrato que retorna `App\Models\User`; paginadores Eloquent tambem atravessam contratos `mixed`. Backlog e regras objetivas foram documentados no Dia 2. | Contratos internos, adapters de Infraestrutura e testes em `tests/Architecture/**`. | Mapa de dependencias, teste arquitetural e suite funcional aprovada. | 2, 3 e 4 | Parcial |
